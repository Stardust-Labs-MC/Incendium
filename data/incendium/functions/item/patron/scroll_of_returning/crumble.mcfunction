...
execute if score $rand in.dummy matches 1.. run tellraw @s {"translate": "%1$s%4733088$s","with":["The ancient technique withstood itself, this time...",{"translate":"incendium.item.returning_scroll.system.withstood"}], "italic": true, "color": "#D393F5"}
...
execute unless score $rand in.dummy matches 1.. run tellraw @s {"translate": "%1$s%4733088$s","with":["The ancient technique crumbled upon usage",{"translate":"incendium.item.returning_scroll.system.crumble"}], "italic": true, "color": "#D393F5"}